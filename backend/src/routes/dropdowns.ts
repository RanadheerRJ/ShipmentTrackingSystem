...
router.get('/:type', (req: AuthedRequest, res) => {
...
router.post('/:type', (req: AuthedRequest, res) => {
  const { type } = req.params;
  ...
  db.prepare(`INSERT INTO ${type} (id, organization_id, name, is_active, created_at) VALUES (?, ?, ?, 1, ?)`)
    .run(id, organization_id ?? actor.organization_id, name, now);
  db.prepare('INSERT INTO audit_logs (id, entity_type, entity_id, action_type, old_value, new_value, performed_by, performed_by_name, organization_id, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(uuidv4(), 'Dropdown', id, 'CREATE', null, JSON.stringify({ type, id, name }), actor.id, actor.name, organization_id ?? actor.organization_id, now);
  res.status(201).json({ id, name });
});

router.delete('/:type/:id', (req: AuthedRequest, res) => {
  const { type, id } = req.params;
  if (!allowedTypes.includes(type)) return res.status(400).json({ error: 'Invalid type' });
  const actor = req.user;
  const item = db.prepare<[string], DropdownRow>(`SELECT * FROM ${type} WHERE id = ?`).get(id);
  if (!item) return res.status(404).json({ error: 'Not found' });
  db.prepare(`UPDATE ${type} SET is_active = 0 WHERE id = ?`).run(id);
  db.prepare('INSERT INTO audit_logs (id, entity_type, entity_id, action_type, old_value, new_value, performed_by, performed_by_name, organization_id, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(uuidv4(), 'Dropdown', id, 'DELETE', JSON.stringify(item), null, actor.id, actor.name, item.organization_id, new Date().toISOString());
